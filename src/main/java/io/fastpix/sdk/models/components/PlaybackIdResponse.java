/* 
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
package io.fastpix.sdk.models.components;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonInclude.Include;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import io.fastpix.sdk.utils.Utils;
import java.lang.Boolean;
import java.lang.Override;
import java.lang.String;
import java.lang.SuppressWarnings;
import java.util.Objects;
import java.util.Optional;

/**
 * PlaybackIdResponse
 * 
 * <p>Displays the result of the request.
 */
public class PlaybackIdResponse {

    /**
     * It demonstrates whether the request is successful or not.
     */
    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("success")
    private Optional<Boolean> success;

    @JsonInclude(Include.NON_ABSENT)
    @JsonProperty("data")
    private Optional<? extends PlaybackIdResponseData> data;

    @JsonCreator
    public PlaybackIdResponse(
            @JsonProperty("success") Optional<Boolean> success,
            @JsonProperty("data") Optional<? extends PlaybackIdResponseData> data) {
        Utils.checkNotNull(success, "success");
        Utils.checkNotNull(data, "data");
        this.success = success;
        this.data = data;
    }
    
    public PlaybackIdResponse() {
        this(Optional.empty(), Optional.empty());
    }

    /**
     * It demonstrates whether the request is successful or not.
     */
    @JsonIgnore
    public Optional<Boolean> success() {
        return success;
    }

    @SuppressWarnings("unchecked")
    @JsonIgnore
    public Optional<PlaybackIdResponseData> data() {
        return (Optional<PlaybackIdResponseData>) data;
    }

    public final static Builder builder() {
        return new Builder();
    }    

    /**
     * It demonstrates whether the request is successful or not.
     */
    public PlaybackIdResponse withSuccess(boolean success) {
        Utils.checkNotNull(success, "success");
        this.success = Optional.ofNullable(success);
        return this;
    }

    /**
     * It demonstrates whether the request is successful or not.
     */
    public PlaybackIdResponse withSuccess(Optional<Boolean> success) {
        Utils.checkNotNull(success, "success");
        this.success = success;
        return this;
    }

    public PlaybackIdResponse withData(PlaybackIdResponseData data) {
        Utils.checkNotNull(data, "data");
        this.data = Optional.ofNullable(data);
        return this;
    }

    public PlaybackIdResponse withData(Optional<? extends PlaybackIdResponseData> data) {
        Utils.checkNotNull(data, "data");
        this.data = data;
        return this;
    }

    
    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        PlaybackIdResponse other = (PlaybackIdResponse) o;
        return 
            Objects.deepEquals(this.success, other.success) &&
            Objects.deepEquals(this.data, other.data);
    }
    
    @Override
    public int hashCode() {
        return Objects.hash(
            success,
            data);
    }
    
    @Override
    public String toString() {
        return Utils.toString(PlaybackIdResponse.class,
                "success", success,
                "data", data);
    }
    
    public final static class Builder {
 
        private Optional<Boolean> success = Optional.empty();
 
        private Optional<? extends PlaybackIdResponseData> data = Optional.empty();
        
        private Builder() {
          // force use of static builder() method
        }

        /**
         * It demonstrates whether the request is successful or not.
         */
        public Builder success(boolean success) {
            Utils.checkNotNull(success, "success");
            this.success = Optional.ofNullable(success);
            return this;
        }

        /**
         * It demonstrates whether the request is successful or not.
         */
        public Builder success(Optional<Boolean> success) {
            Utils.checkNotNull(success, "success");
            this.success = success;
            return this;
        }

        public Builder data(PlaybackIdResponseData data) {
            Utils.checkNotNull(data, "data");
            this.data = Optional.ofNullable(data);
            return this;
        }

        public Builder data(Optional<? extends PlaybackIdResponseData> data) {
            Utils.checkNotNull(data, "data");
            this.data = data;
            return this;
        }
        
        public PlaybackIdResponse build() {
            return new PlaybackIdResponse(
                success,
                data);
        }
    }
}
