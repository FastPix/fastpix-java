package io.fastpix.sdk.models.operations;

import com.fasterxml.jackson.annotation.JsonCreator;
import jakarta.annotation.Nonnull;
import jakarta.annotation.Nullable;
import java.lang.Override;
import java.lang.String;
import java.util.Optional;
import io.fastpix.sdk.models.components.MediaIdsRequest;
import io.fastpix.sdk.utils.FastpixMetadata;
import io.fastpix.sdk.utils.Utils;


public class DeleteMediaFromPlaylistRequest {
    /**
     * The unique id of the playlist you want to perform the operation on.
     */
    @FastpixMetadata("pathParam:style=simple,explode=false,name=playlistId")
    private String playlistId;


    @FastpixMetadata("request:mediaType=application/json")
    private MediaIdsRequest body;

    @JsonCreator
    public DeleteMediaFromPlaylistRequest(
            @Nonnull String playlistId,
            @Nullable MediaIdsRequest body) {
        this.playlistId = Optional.ofNullable(playlistId)
            .orElseThrow(() -> new IllegalArgumentException("playlistId cannot be null"));
        this.body = body;
    }
    
    public DeleteMediaFromPlaylistRequest(
            @Nonnull String playlistId) {
        this(playlistId, null);
    }

    /**
     * The unique id of the playlist you want to perform the operation on.
     */
    public String playlistId() {
        return this.playlistId;
    }

    public Optional<MediaIdsRequest> body() {
        return Optional.ofNullable(this.body);
    }

    public static Builder builder() {
        return new Builder();
    }


    /**
     * The unique id of the playlist you want to perform the operation on.
     */
    public DeleteMediaFromPlaylistRequest withPlaylistId(@Nonnull String playlistId) {
        this.playlistId = Utils.checkNotNull(playlistId, "playlistId");
        return this;
    }


    public DeleteMediaFromPlaylistRequest withBody(@Nullable MediaIdsRequest body) {
        this.body = body;
        return this;
    }


    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        DeleteMediaFromPlaylistRequest other = (DeleteMediaFromPlaylistRequest) o;
        return 
            Utils.enhancedDeepEquals(this.playlistId, other.playlistId) &&
            Utils.enhancedDeepEquals(this.body, other.body);
    }
    
    @Override
    public int hashCode() {
        return Utils.enhancedHash(
            playlistId, body);
    }
    
    @Override
    public String toString() {
        return Utils.toString(DeleteMediaFromPlaylistRequest.class,
                "playlistId", playlistId,
                "body", body);
    }

    @SuppressWarnings("UnusedReturnValue")
    public final static class Builder {

        private String playlistId;

        private MediaIdsRequest body;

        private Builder() {
          // force use of static builder() method
        }

        /**
         * The unique id of the playlist you want to perform the operation on.
         */
        public Builder playlistId(@Nonnull String playlistId) {
            this.playlistId = Utils.checkNotNull(playlistId, "playlistId");
            return this;
        }

        public Builder body(@Nullable MediaIdsRequest body) {
            this.body = body;
            return this;
        }

        public DeleteMediaFromPlaylistRequest build() {
            return new DeleteMediaFromPlaylistRequest(
                playlistId, body);
        }

    }
}
