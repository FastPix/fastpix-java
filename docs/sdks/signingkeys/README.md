...
<!-- UsageSnippet language="java" operationID="create_signing_key" method="post" path="/iam/signing-keys" -->
```java
...
public class Application {
    ...
    public static void main(String[] args) throws Exception {
        ...
        CreateSigningKeyResponse res = sdk.signingKeys().create()
                .call();

        if (res.createResponse().isPresent()) {
            var mapper = JSON.getMapper();
            mapper.enable(SerializationFeature.INDENT_OUTPUT);
            System.out.println(mapper.writeValueAsString(res.createResponse().get()));
        }
    }
}
```
...
<!-- UsageSnippet language="java" operationID="list_signing_keys" method="get" path="/iam/signing-keys" -->
```java
// Package declaration - adjust to match your project's directory structure
package hello.world;
...
public class Application {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ListSigningKeysResponse res = sdk.signingKeys().list()
                .limit(25L)
                .offset(1L)
                .call();

        if (res.getAllSigningKeysResponse().isPresent()) {
            var mapper = JSON.getMapper();
            mapper.enable(SerializationFeature.INDENT_OUTPUT);
            System.out.println(mapper.writeValueAsString(res.getAllSigningKeysResponse().get()));
        }
    }
}
```
...
| ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `limit`                                                                       | *Optional\<Long>*                                                             | :heavy_minus_sign:                                                            | Limit specifies the maximum number of items to display per page.              | 25                                                                            |
| `offset`                                                                      | *Optional\<Long>*                                                             | :heavy_minus_sign:                                                            | It is used for pagination, indicating the starting point for fetching data.   | 1                                                                             |

### Response

**[ListSigningKeysResponse](../../models/operations/ListSigningKeysResponse.md)**
...
<!-- UsageSnippet language="java" operationID="delete_signing_key" method="delete" path="/iam/signing-keys/{signingKeyId}" -->
```java
// Package declaration - adjust to match your project's directory structure
package hello.world;
...
<!-- UsageSnippet language="java" operationID="get-signing_key_by_id" method="get" path="/iam/signing-keys/{signingKeyId}" -->
```java
// Package declaration - adjust to match your project's directory structure
package hello.world;
...
public class Application {
    ...
    public static void main(String[] args) throws Exception {
        ...
        GetSigningKeyByIdResponse res = sdk.signingKeys().getById()
                .signingKeyId("your-signing-key-id")
                .call();

        if (res.getPublicPemUsingSigningKeyIdResponseDTO().isPresent()) {
            var mapper = JSON.getMapper();
            mapper.enable(SerializationFeature.INDENT_OUTPUT);
            System.out.println(mapper.writeValueAsString(res.getPublicPemUsingSigningKeyIdResponseDTO().get()));
        }
    }
}
```
...
**[GetSigningKeyByIdResponse](../../models/operations/GetSigningKeyByIdResponse.md)**
